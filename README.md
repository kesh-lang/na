...
In other words, the character `_` is permitted anywhere in an identifier. An identifier may contain but not start or end with `-`. The characters `_` and `-` are interchangeable: `foo-bar` is equivalent to `foo_bar`.
